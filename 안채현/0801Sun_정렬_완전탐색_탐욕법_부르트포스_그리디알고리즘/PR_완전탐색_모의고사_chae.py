def solution(answers):
    ...
    score2 = 0
    score3 = 0
#IndexError: list index out of range??      ##에러가 난 이유!!: answer이 아니라  answers!
    for i in range(len(answers)): #제공된 answers의 개수. i: range(5) >> i: 0 1 2 3 4
        print(i)   #0 1 2 3 4
        if psn1[i%len1] == answers[i]:   #i:0 > psn1[0] == answer[0] // i:1 > psn1[1] == answer[1]
            score1 += 1
        if psn2[i%len2] == answers[i]:   #i:0 > psn2[0] == answer[0] // i:1 > psn2[1] == answer[1] //i:7 > psn2[7을 psn2 len인 8로 나눈 나머지, 7번째자리값과 
            score2 += 1
        if psn3[i%len3] == answers[i]:   #i:0 > psn1[0] == answer[0] // i:1 > psn1[1] == answer[1]
            score3 += 1
